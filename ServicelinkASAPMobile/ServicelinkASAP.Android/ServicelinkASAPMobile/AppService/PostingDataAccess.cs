...
namespace ServicelinkASAPMobile
{
    public class PostingDataAccess
    {
		...
		public void DeletePostingByID_Async(string orderID, CancellationToken cancellationToken = default(CancellationToken)){
			var _p = Database.GetConnection (cancellationToken).Table<Posting> ().Where (p => p.OrderID.Equals(orderID));
			Database.GetConnection (cancellationToken).DeleteAsync (_p.FirstAsync());
